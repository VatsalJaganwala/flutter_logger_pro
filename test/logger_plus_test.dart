...
void main() {
  group('Logger Tests', () {
    ...
    test('Logger JSON methods can be called without throwing', () {
      ...
      final testMap = {'key': 'value', 'number': 42, 'nested': {'inner': true}};
      final testList = [1, 'two', {'three': 3}, [4, 5]];
  ...
  group('LoggerOptions Singleton Tests', () {
    ...
    test('LoggerOptions has correct default values', () {
      ...
      expect(options.effectiveShowClassName, isFalse); // Disabled for future release
    ...
    test('LoggerOptions configure method handles null parameters correctly', () {
      final options = LoggerOptions.instance;
  ...
  group('Configuration Precedence Tests', () {
    ...
    test('Logger constructor parameters override global display control settings', () {
      LoggerOptions.instance.configure(
        // showClassName: false, // Hidden for future release
        showFunctionName: false,
        showLocation: false,
    ...
    test('Logger constructor parameters override global settings completely', () {
    ...
    test('Logger uses global settings when constructor parameters not provided', () {
      // Set global settings
      LoggerOptions.instance.configure(
        enableLogging: false,
        enableColors: false,
  ...
  group('Logger Integration Tests', () {
    ...
    test('Multiple loggers with different configurations work independently', () {
      LoggerOptions.instance.configure(
        enableLogging: false,
        enableColors: false,
      ...
      final disabledLogger = Logger(tag: 'DISABLED'); // Uses global (disabled)
      final enabledLogger = Logger(tag: 'ENABLED', enableLogging: true); // Override
      final colorfulLogger = Logger(tag: 'COLORFUL', enableColors: true); // Partial override
  ...
  group('Edge Cases and Error Handling Tests', () {
    ...
    test('Logger creation with all possible constructor combinations', () {
      ...
      final logger5 = Logger(/* showClassName: true, */ ); // Hidden for future release
      ...
      final loggers = [logger1, logger2, logger3, logger4, logger5, logger6, logger7, logger8];
  ...
  group('LoggerOptions Validation and Error Handling Tests', () {
    ...
    test('LoggerOptions validates LogLevel values correctly', () {
      ...
      expect(() => options.configure(minLogLevel: LogLevel.debug), returnsNormally);
      expect(() => options.configure(minLogLevel: LogLevel.info), returnsNormally);
      expect(() => options.configure(minLogLevel: LogLevel.warn), returnsNormally);
      expect(() => options.configure(minLogLevel: LogLevel.error), returnsNormally);
    ...
    test('LoggerOptions handles invalid dateTimeFormat gracefully', () {
      ...
      expect(() => options.configure(dateTimeFormat: 'HH:mm:ss'), returnsNormally);
      expect(options.effectiveDateTimeFormat, equals('HH:mm:ss'));
      
      expect(() => options.configure(dateTimeFormat: 'yyyy-MM-dd HH:mm:ss'), returnsNormally);
      expect(options.effectiveDateTimeFormat, equals('yyyy-MM-dd HH:mm:ss'));
      ...
      expect(() => options.configure(dateTimeFormat: 'invalid@#\$'), returnsNormally);
      expect(options.effectiveDateTimeFormat, equals('HH:mm:ss')); // Default
    });
    ...
    test('LoggerOptions validates messageTemplate and provides warnings', () {
      ...
      expect(() => options.configure(messageTemplate: '{level}: {message}'), returnsNormally);
      expect(options.effectiveMessageTemplate, equals('{level}: {message}'));
      
      expect(() => options.configure(messageTemplate: '[{timestamp}] {level} - {message}'), returnsNormally);
      expect(options.effectiveMessageTemplate, equals('[{timestamp}] {level} - {message}'));
      
      expect(() => options.configure(messageTemplate: 'Simple message: {message}'), returnsNormally);
      expect(options.effectiveMessageTemplate, equals('Simple message: {message}'));
      
      // Invalid templates should still be set but with warnings
      expect(() => options.configure(messageTemplate: ''), returnsNormally);
      expect(options.effectiveMessageTemplate, equals('')); // Still set despite being invalid
      
      expect(() => options.configure(messageTemplate: '{invalidPlaceholder}'), returnsNormally);
      expect(options.effectiveMessageTemplate, equals('{invalidPlaceholder}')); // Still set with warning
    });
    ...
    test('LoggerOptions handles valid messageTemplate placeholders', () {
      ...
      for (final template in validTemplates) {
        expect(() => options.configure(messageTemplate: template), returnsNormally);
    ...
    test('LoggerOptions validation does not throw exceptions for user errors', () {
      final options = LoggerOptions.instance;
      ...
      expect(() => options.configure(messageTemplate: '{invalid}'), returnsNormally);
    });

    test('LoggerOptions validation preserves valid settings when invalid ones are provided', () {
      final options = LoggerOptions.instance;
      ...
      expect(options.effectiveDateTimeFormat, equals('HH:mm:ss')); // Reverted to default
      expect(options.effectiveMessageTemplate, equals('{level}: {message}')); // Unchanged
    });
    ...
    test('LoggerOptions handles mixed valid and invalid configuration', () {
      ...
      expect(options.effectiveEnableLogging, isTrue);
      expect(options.effectiveEnableColors, isFalse);
      expect(options.effectiveDateTimeFormat, equals('HH:mm:ss')); // Default due to invalid input
      expect(options.effectiveMessageTemplate, equals('{level}: {message}'));
      expect(options.effectiveIncludeTimestamp, isTrue);
    });
    ...
    test('LoggerOptions validation edge cases', () {
      ...
      expect(() => options.configure(dateTimeFormat: ' '), returnsNormally); // Whitespace
      expect(() => options.configure(dateTimeFormat: '\t\n'), returnsNormally); // Tabs/newlines
      expect(() => options.configure(messageTemplate: ' '), returnsNormally); // Whitespace
      expect(() => options.configure(messageTemplate: '{}'), returnsNormally); // Empty placeholder
    ...
    test('LoggerConfigurationException is properly defined', () {
      ...
      expect(exception.toString(), equals('LoggerConfigurationException: Test message'));
    ...
    test('LoggerOptions handles complex dateTimeFormat validation', () {
      ...
      for (final format in validFormats) {
        ...
        expect(options.effectiveDateTimeFormat, equals(format), 
    ...
    test('LoggerOptions handles complex messageTemplate validation', () {
      ...
      for (final template in invalidTemplates) {
        options.configure(messageTemplate: template);
        expect(options.effectiveMessageTemplate, equals(template), 
               reason: 'Invalid template "$template" should still be set with warning');
      }
    });
    ...
    test('LoggerOptions validation preserves application state during errors', () {
      final options = LoggerOptions.instance;
      ...
      expect(options.effectiveMessageTemplate, equals('{invalidPlaceholder}')); // Set with warning
    });
  });
