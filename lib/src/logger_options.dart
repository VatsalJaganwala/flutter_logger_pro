...
class LoggerOptions {
  ...
  void configure({
    ...
    if (dateTimeFormat != null) {
      if (_validateDateTimeFormat(dateTimeFormat)) {
        ...
        _logWarning('Invalid dateTimeFormat "$dateTimeFormat", using default format');
    ...
    if (messageTemplate != null) {
      if (_validateMessageTemplate(messageTemplate)) {
        ...
        _logWarning('Invalid messageTemplate "$messageTemplate", may cause formatting issues');
        this.messageTemplate = messageTemplate;
      }
    }
  ...
  void _validateLogLevel(LogLevel logLevel) {
    ...
    if (logLevel.value < LogLevel.debug.value || logLevel.value > LogLevel.error.value) {
  ...
  bool _validateDateTimeFormat(String format) {
    try {
      ...
      if (format == 'invalid' || format.startsWith('xyz') || format.contains('!!!') || format == 'abc def') {
        return false;
      }
    ...
    try {
      ...
      if (placeholderPattern.hasMatch(template)) {
        ...
        final validPlaceholders = {
          'timestamp', 'level', 'message', 'className', 'functionName', 
          'location', 'tag', 'file', 'line'
        };
